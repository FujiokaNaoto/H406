...
public class CLinear : MonoBehaviour {
    ...
    protected void OnNext() {
        //CPanel next = null;
		CFieldPanel next = null;
        switch (directions[0]) {
            case CPanel.EDirection.UP:      next = (CFieldPanel)panel.up;    break;
			case CPanel.EDirection.LEFT:	next = (CFieldPanel)panel.left; break;
			case CPanel.EDirection.DOWN:	next = (CFieldPanel)panel.down; break;
			case CPanel.EDirection.RIGHT:	next = (CFieldPanel)panel.right; break;
        }
        if (next == null) {
            panel = null;
            return;
		...
		CAudio.Instance.PlaySE(CAudio.SECODE.LINEAR);
    }
